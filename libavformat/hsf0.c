...
{
    if (AV_RB32(p->buf) != MKBETAG('H','S','F','\0') &&
        AV_RB32(p->buf) != MKBETAG('H','S','F',' '))
        return 0;
...
{
    int rate, align, version;
    AVIOContext *pb = s->pb;
    AVStream *st;

    avio_skip(pb, 3);
    version = (avio_r8(pb) == '\0') ? 1 : 0;
    avio_skip(pb, 4);
    rate = avio_rl32(pb);
    if (version == 1)
...
const FFInputFormat ff_hsf0_demuxer = {
    .p.name         = "hsf0",
    .p.long_name    = NULL_IF_CONFIG_SMALL("SoundBox HSF"),

...
{
    if (AV_RB32(p->buf) != MKBETAG('M','S','V','p'))
        return 0;
...
static int read_header(AVFormatContext *s)
{
    ...
    st->codecpar->bit_rate = 16LL * 8 * rate / 28;
...
const FFInputFormat ff_msvp_demuxer = {
    .p.name         = "msvp",
    .p.long_name    = NULL_IF_CONFIG_SMALL("Sony MultiStream MSV"),

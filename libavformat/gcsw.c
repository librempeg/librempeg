...
{
    if (AV_RB32(p->buf) != MKBETAG('G','C','S','W'))
        return 0;
...
static int read_header(AVFormatContext *s)
{
    ...
    if (rate <= 0 || channels <= 0 || channels > INT_MAX/0x8000)
...
const FFInputFormat ff_gcsw_demuxer = {
    .p.name         = "gcsw",
    .p.long_name    = NULL_IF_CONFIG_SMALL("MileStone GCSW"),

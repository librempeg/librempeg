...
static int scdstm_read_header(AVFormatContext *s)
{
    ...
    if (scd->audio) {
        ...
        ast->codecpar->codec_id = AV_CODEC_ID_PCM_SU8;

        ast->codecpar->block_align = 0x8000;
        ast->codecpar->sample_rate = 16384;
        ast->codecpar->ch_layout.nb_channels = 2;

        ast->duration = avio_size(pb) / (ast->codecpar->block_align + vst->codecpar->block_align);
        vst->duration = ast->duration;
        avpriv_set_pts_info(ast, 64, 1, ast->codecpar->sample_rate);
    } else
        vst->duration = avio_size(pb) / vst->codecpar->block_align;
...
static int scdstm_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    avio_seek(pb, pos + s->streams[stream_idx]->codecpar->block_align, SEEK_SET);
    printf("packet fin\n");

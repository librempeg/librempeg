...
static int read_probe(const AVProbeData *p)
{
    ...
    if (AV_RB32(p->buf) != MKBETAG('R','A','S','_'))
        return 0;
    ...
    if ((int)AV_RB32(p->buf+20) <= 0)
        return 0;
    if ((int)AV_RB32(p->buf+32) <= 0)
        return 0;

    for (int ch = 0; ch < 2; ch++) {
        for (int n = 0; n < 16; n++)
            score += 3 * (AV_RN16(p->buf + 64 + n * 2 + (32 + 16) * ch) != 0);
    }
...
const FFInputFormat ff_ras_demuxer = {
    .p.name         = "ras",
    .p.long_name    = NULL_IF_CONFIG_SMALL("Retro RAS"),

...
typedef struct ACXDemuxContext {
    int current_stream;
} ACXDemuxContext;

typedef struct ACXStream {
...
static int read_probe(const AVProbeData *p)
{
    ...
    if (av_match_ext(p->filename, "acx") == 0)
        return 0;
...
static int sort_streams(const void *a, const void *b)
{
    ...
    const ACXStream *as1 = s1->priv_data;
    const ACXStream *as2 = s2->priv_data;
...
static int read_header(AVFormatContext *s)
{
    ...
    for (int i = 0; i < nb_streams; i++) {
        int64_t start_offset = avio_rb32(pb);
        ACXStream *ast;
        AVStream *st;
...
{
    ACXDemuxContext *acx = s->priv_data;
    ACXStream *ast;
    AVStream *st;

    acx->current_stream = av_clip(stream_index, 0, s->nb_streams-1);
    st = s->streams[acx->current_stream];
    ast = st->priv_data;
...
const FFInputFormat ff_acx_demuxer = {
    .p.name         = "acx",
    .p.long_name    = NULL_IF_CONFIG_SMALL("CRI ACX"),
    .flags_internal = FF_INFMT_FLAG_INIT_CLEANUP,
    .p.flags        = AVFMT_GENERIC_INDEX,
    .p.extensions   = "acx",
    .priv_data_size = sizeof(ACXDemuxContext),

...
{
    if (AV_RB32(p->buf) != MKBETAG('W','A','V','2'))
        return 0;

    if (p->buf_size < 28)
        return 0;
    if ((int)AV_RL16(p->buf+12) <= 0)
        return 0;
...
const FFInputFormat ff_wav2_demuxer = {
    .p.name         = "wav2",
    .p.long_name    = NULL_IF_CONFIG_SMALL("Infogrames North America WAV2"),

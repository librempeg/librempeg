...
{
    if (AV_RB32(p->buf) != MKBETAG('S','V','G','p'))
        return 0;

    if (p->buf_size < 38)
        return 0;
    if (AV_RL32(p->buf+0x18) == 0)
        return 0;
    if ((int)AV_RL32(p->buf+0x14) <= 0)
        return 0;
    if ((int)AV_RB32(p->buf+0x2c) <= 0)
        return 0;
...
const FFInputFormat ff_svgp_demuxer = {
    .p.name         = "svgp",
    .p.long_name    = NULL_IF_CONFIG_SMALL("High Voltage SVGp"),

...
{
    if (AV_RB32(p->buf) != MKBETAG('M','C','S','S'))
        return 0;
...
static int read_header(AVFormatContext *s)
{
    ...
    align = avio_rl32(pb);
    if (align <= 0 || rate <= 0 || channels <= 0 || align >= INT_MAX/channels)
...
const FFInputFormat ff_mcss_demuxer = {
    .p.name         = "mcss",
    .p.long_name    = NULL_IF_CONFIG_SMALL("Guerrilla MSS"),

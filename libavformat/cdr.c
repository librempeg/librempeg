...
static int cdr_probe(const AVProbeData *p)
{
    ...
    for (int n = 4; n < p->buf_size - 4; n += 4) {
        const int l0 = sign_extend(AV_RB16(buf + n - 4), 16);
        const int r0 = sign_extend(AV_RB16(buf + n - 2), 16);
        const int l1 = sign_extend(AV_RB16(buf + n + 0), 16);
        const int r1 = sign_extend(AV_RB16(buf + n + 2), 16);

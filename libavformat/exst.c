...
static int read_header(AVFormatContext *s)
{
    ...
    rate = avio_rl32(pb);
    if (rate <= 0 || nb_channels <= 0 || nb_channels > INT_MAX/0x400)
    ...
    avio_seek(pb, 0x78, SEEK_SET);

    return 0;
}

const FFInputFormat ff_exst_demuxer = {
    .p.name         = "exst",
    .p.long_name    = NULL_IF_CONFIG_SMALL("Sony EXST"),

...
{
    if (AV_RB32(p->buf) != MKBETAG('V','G','S','\x0'))
        return 0;
    if (p->buf_size < 20)
        return 0;
...
static int read_header(AVFormatContext *s)
{
    ...
    avio_skip(pb, 4);
    align = avio_rb32(pb) * 0x1000;
    avio_skip(pb, 8);
    rate = avio_rb32(pb);
    if (align <= 0 || rate <= 0 || align > INT_MAX/2)
...
const FFInputFormat ff_vgs_demuxer = {
    .p.name         = "vgs",
    .p.long_name    = NULL_IF_CONFIG_SMALL("VGS PS2"),
    .p.extensions   = "vgs",

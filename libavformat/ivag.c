...
{
    if (AV_RB32(p->buf) != MKBETAG('I','V','A','G'))
        return 0;
...
static int read_header(AVFormatContext *s)
{
    ...
    avio_seek(pb, 0x40 + 0x40LL * channels, SEEK_SET);

    return 0;
}

const FFInputFormat ff_ivag_demuxer = {
    .p.name         = "ivag",
    .p.long_name    = NULL_IF_CONFIG_SMALL("Namco IVAG"),

...
{
    if (AV_RB32(p->buf) != MKBETAG('A','S','T','\0'))
        return 0;
...
static int read_header(AVFormatContext *s)
{
    ...
    avio_seek(pb, 0x100, SEEK_SET);

    return 0;
}

const FFInputFormat ff_ast0_demuxer = {
    .p.name         = "ast0",
    .p.long_name    = NULL_IF_CONFIG_SMALL("Marvelous AST"),

...
{
    if (AV_RB32(p->buf) != MKBETAG('A','H','V','\0'))
        return 0;
    ...
    if ((int)AV_RL32(p->buf + 12) <= 0)
        return 0;
    if (AV_RB32(p->buf+20) != MKBETAG('V','A','G','p'))
        return 0;
...
const FFInputFormat ff_ahv_demuxer = {
    .p.name         = "ahv",
    .p.long_name    = NULL_IF_CONFIG_SMALL("Amuze AHV"),

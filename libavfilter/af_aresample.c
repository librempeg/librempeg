...
static int filter_frame(AVFilterLink *inlink, AVFrame *insamplesref)
{
    ...
    int n_out = swr_get_out_samples(aresample->swr, n_in);
...
static int flush_frame(AVFilterLink *outlink, AVFrame **outsamplesref_ret)
{
    AVFilterContext *ctx = outlink->src;
    AResampleContext *aresample = ctx->priv;
    ...
    n_out = swr_get_out_samples(aresample->swr, 0);
    outsamplesref = ff_get_audio_buffer(outlink, n_out);
...
static int activate(AVFilterContext *ctx)
{
    ...
    if (aresample->eof) {
        ...
        if ((ret = flush_frame(outlink, &outsamplesref)) < 0) {
            if (ret == AVERROR_EOF) {

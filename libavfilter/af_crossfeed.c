...
static int filter_frame(AVFilterLink *inlink, AVFrame *in)
{
    ...
    if (s->block_samples == 0) {
        ...
        sdst = s->side[1];
        filter_samples(sdst, ssrc, nb_samples,
                       b0, b1, b2, a0, a1, a2,
                       &w1, &w2);
        ...
        reverse_samples(s->side[2], s->side[1], nb_samples * 2);
        w1 = w2 = 0.;
        filter_samples(s->side[2], s->side[2], nb_samples * 2,
                       b0, b1, b2, a0, a1, a2,
                       &w1, &w2);

        reverse_samples(s->side[1], s->side[2], nb_samples * 2);

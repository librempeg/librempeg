...
struct integrator {
    ...
    unsigned cache_pos;             ///< focus on the last added bin in the cache array
    unsigned cache_size;            ///< window cache size
    double *sum;                    ///< sum of the last N ms filtered samples (cache content)
    unsigned filled;                ///< 1 if the cache is completely filled, 0 otherwise
    double rel_threshold;           ///< relative threshold
    double sum_kept_powers;         ///< sum of the powers (weighted sums) above absolute threshold
    unsigned nb_kept_powers;        ///< number of sum above absolute threshold
...
typedef struct EBUR128Context {
    ...
    TPKState *true_peak_state;      ///< true peak state per channel
    ...
    double (*samples_peak)(const void *src, const int nb_samples);
    double (*samples_true_peak)(const void *src, const int nb_samples, TPKState *state);
...
#define WEIGHT_1_41_MASK (AV_CH_BACK_LEFT             |AV_CH_BACK_CENTER          | \
    ...
    case AV_SAMPLE_FMT_DBLP:
        ebur128->samples_peak = samples_peak_dblp;
        ebur128->samples_true_peak = samples_true_peak_dblp;
        ebur128->ch_samples.f64 = av_calloc(nb_channels, sizeof(*ebur128->ch_samples.f64));
        if (!ebur128->ch_samples.f64)
            return AVERROR(ENOMEM);
        break;
    case AV_SAMPLE_FMT_FLTP:
        ebur128->samples_peak = samples_peak_fltp;
        ebur128->samples_true_peak = samples_true_peak_fltp;
        ebur128->ch_samples.f32 = av_calloc(nb_channels, sizeof(*ebur128->ch_samples.f32));
        if (!ebur128->ch_samples.f32)
            return AVERROR(ENOMEM);
        break;
    case AV_SAMPLE_FMT_S16P:
        ebur128->samples_peak = samples_peak_s16p;
        ebur128->samples_true_peak = samples_true_peak_s16p;
    ...
    case AV_SAMPLE_FMT_S32P:
        ...
        ebur128->samples_true_peak = samples_true_peak_s32p;
...
#define I3000_BINS(x) ((x) * 3)
    ...
    if (ebur128->peak_mode & PEAK_MODE_TRUE_PEAKS) {
        ...
        ebur128->true_peak_state = av_calloc(nb_channels, sizeof(*ebur128->true_peak_state));
        ebur128->true_peaks_per_frame = av_calloc(nb_channels, sizeof(*ebur128->true_peaks_per_frame));
        if (!ebur128->true_peaks || !ebur128->true_peak_state || !ebur128->true_peaks_per_frame)
            return AVERROR(ENOMEM);
    }
                                 ...
                                 const int nb_samples)
{
    if (ebur128->peak_mode & PEAK_MODE_TRUE_PEAKS) {
        ...
        for (int ch = 0; ch < nb_channels; ch++) {
            const void *src = csamples[ch];
            TPKState *true_peak_state = &ebur128->true_peak_state[ch];
            double true_peak = ebur128->true_peaks[ch];
            double true_peak_per_frame = ebur128->samples_true_peak(src, nb_samples, true_peak_state);
    ...
    if (ebur128->peak_mode & PEAK_MODE_SAMPLES_PEAKS) {
        ...
        for (int ch = 0; ch < nb_channels; ch++) {
            ...
            double sample_peak_per_frame = ebur128->samples_peak(src, nb_samples);
...
static int filter_frame(AVFilterLink *inlink, AVFrame *insamples)
{
    ...
    unsigned *i400_filled = &i400->filled;
    ...
    case AV_SAMPLE_FMT_DBLP:
        for (int ch = 0; ch < nb_channels; ch++)
            ch_samples_f64[ch] = ((const double *)samples[ch]) + idx_insample;
        break;
    case AV_SAMPLE_FMT_FLTP:
        for (int ch = 0; ch < nb_channels; ch++)
            ch_samples_f32[ch] = ((const float *)samples[ch]) + idx_insample;
        break;
    case AV_SAMPLE_FMT_S16P:
        for (int ch = 0; ch < nb_channels; ch++)
    ...
    while (samples_to_process > 0) {
        switch (inlink->format) {
        case AV_SAMPLE_FMT_DBLP:
            process_block_dblp(ch_samples_f64, nb_channels,
            ...
            break;
        case AV_SAMPLE_FMT_FLTP:
            process_block_fltp(ch_samples_f32, nb_channels,
                               i3000_cache_size, i400_cache_size,
                               i3000_cache_pos, i400_cache_pos,
                               i3000_filled, i400_filled,
                               ch_weighting, cf,
...
static av_cold void uninit_ebur128(AVFilterContext *ctx, EBUR128Context *ebur128)
{
    ...
    av_freep(&ebur128->true_peak_state);
...
static int loudnorm_filter_frame(AVFilterLink *inlink, AVFrame *in)
{
    ...
    const unsigned i3000_cache_size = i3000->cache_size;
    const unsigned i400_cache_size = i400->cache_size;
    const double **ch_samples_f64 = r128_in->ch_samples.f64;
    ...
    for (int ch = 0; ch < nb_channels && in; ch++)
        ch_samples_f64[ch] = ((const double *)samples[ch]) + r128_in->idx_insample;

    for (int idx_insample = r128_in->idx_insample; idx_insample < nb_samples; idx_insample++) {
        process_ebur128_dblp(ch_samples_f64, nb_channels,
                             i3000_cache_size, i400_cache_size,
                             i3000_cache_pos, i400_cache_pos,
                             i3000_filled, i400_filled,
                             ch_weighting, cf,
...

    {
        ...
        const unsigned i3000_cache_size = i3000->cache_size;
        const unsigned i400_cache_size = i400->cache_size;
        const double **ch_samples_f64 = r128_out->ch_samples.f64;
        ...
        for (int ch = 0; ch < nb_channels; ch++)
            ch_samples_f64[ch] = ((const double *)samples[ch]) + r128_out->idx_insample;

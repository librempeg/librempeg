            ...
            for (int i = 0; i < link->incfg.formats->nb_formats; i++) {
                ...
                if (link->incfg.formats->flags != 0) {
                    ...
                    if (link->incfg.formats->flags & FILTER_SAME_BITDEPTH) {
                        ...
                        const int afactor = 1+3*(!!(a->flags & AV_PIX_FMT_FLAG_BAYER));
...
static int pick_formats(AVFilterGraph *graph)
{
    ...
    do {
        ...
        for (int i = 0; i < graph->nb_filters; i++) {
            ...
            if (filter->nb_inputs && filter->nb_outputs && filter->outputs[0]->sample_rate>0) {
                for (int j = 0; j < filter->nb_inputs; j++) {
                    if (filter->inputs[j]->incfg.samplerates && filter->inputs[j]->sample_rate<=0) {
                        if ((ret = pick_samplerate(filter->inputs[j], filter->outputs[0])) < 0)
                            return ret;
                        change = 1;
                    }
                }

...
static int mjpeg_parse_len(MJpegDecodeContext *s, GetByteContext *gB, int *plen, const char *name)
{
    int len = bytestream2_get_be16u(gB);
    if (len < 2 || bytestream2_get_bytes_left(gB) < (len - 2)) {
...
int ff_mjpeg_decode_dqt(MJpegDecodeContext *s)
{
    ...
    int ret = mjpeg_parse_len(s, gB, &len, "dqt");
    if (ret < 0)
        return ret;

    while (len >= 65) {
        uint8_t b = bytestream2_get_byteu(gB);
        int pr = b >> 4;
        if (pr > 1) {
        ...
        /* read quant table */
        for (i = 0; i < 64; i++) {
            s->quant_matrixes[index][i] = pr ? bytestream2_get_be16u(gB) : bytestream2_get_byteu(gB);
...
int ff_mjpeg_decode_dht(MJpegDecodeContext *s)
{
    ...
    ret = mjpeg_parse_len(s, gB, &len, "dht");
    if (ret < 0)
        return ret;
    ...
    while (len > 0) {
        ...
        n = 0;
        for (i = 1; i <= 16; i++) {
            bits_table[i] = bytestream2_get_byteu(gB);
            n += bits_table[i];
        }
        len -= 17;
        if (len < n || n > 256)
...
int ff_mjpeg_decode_sof(MJpegDecodeContext *s)
{
    ...
    ret = mjpeg_parse_len(s, gB, &len, "sof");
    if (ret < 0)
        return ret;
    if (len < 6)
        return AVERROR_INVALIDDATA;
    ...
    nb_components = bytestream2_get_byteu(gB);
    if (nb_components <= 0 ||
    ...
    for (i = 0; i < nb_components; i++) {
        /* component id */
        s->component_id[i] = bytestream2_get_byteu(gB);
        uint8_t b = bytestream2_get_byteu(gB);
        h_count[i]         = b >> 4;
        v_count[i]         = b & 0x0F;
        ...
        if (v_count[i] > s->v_max)
            s->v_max = v_count[i];
        s->quant_index[i] = bytestream2_get_byteu(gB);
...
static int handle_rstn(MJpegDecodeContext *s, int nb_components)
{
    ...
    if (s->restart_interval) {
        ...
        i = 8 + ((-get_bits_count(gb)) & 7);
        /* skip RSTn */
        if (s->restart_count == 0) {
            if(   show_bits(gb, i) == (1 << i) - 1
               || show_bits(gb, i) == 0xFF) {
                int pos = get_bits_count(gb);
                align_get_bits(gb);
                while (get_bits_left(gb) >= 8 && show_bits(gb, 8) == 0xFF)
                    skip_bits(gb, 8);
                if (get_bits_left(gb) >= 8 && (get_bits(gb, 8) & 0xF8) == 0xD0) {
                    for (i = 0; i < nb_components; i++) /* reset dc */
                        s->last_dc[i] = (4 << s->bits);
                    reset = 1;
                } else
                             ...
                             const AVFrame *reference)
{
    ...
    GetBitContext *gb = &s->gb;
    const int progressive = s->progressive;
    const int lowres = s->avctx->lowres;
    const int height = s->height;
    const int width = s->width;
    int16_t *block = s->block;
    const int *nb_blocks = s->nb_blocks;
    const int *comp_index = s->comp_index;
    const int *h_scount = s->h_scount;
    const int *v_scount = s->v_scount;
    ...
    av_pix_fmt_get_chroma_sub_sample(s->avctx->pix_fmt, &chroma_h_shift,
                                     &chroma_v_shift);
    chroma_width  = AV_CEIL_RSHIFT(width,  chroma_h_shift);
    chroma_height = AV_CEIL_RSHIFT(height, chroma_v_shift);
                ...
                if (!progressive) {
                    for (int j = 0; j < n; j++) {
                        ...
                        if (copy_mb && ptr) {
                            ...
                            if (ptr && linesizec) {
                                s->idsp.idct_put(ptr, linesizec, block);
                                if (s->bits & 7)
                                    shift_output(s, ptr, linesizec);
                            }
                        }
                        ...
                        int mb_bitmask_size, const AVFrame *reference)
{
    ...
    ret = mjpeg_parse_len(s, gB, &len, "sos");
    if (ret < 0)
        return ret;
    if (len < 1)
        return AVERROR_INVALIDDATA;
    ...
    predictor = bytestream2_get_byteu(gB);  /* JPEG Ss / lossless JPEG predictor / JPEG-LS NEAR */
    ilv = bytestream2_get_byteu(gB);        /* JPEG Se / JPEG-LS ILV */
    ...
    if (s->mjpb_skiptosod)
        bytestream2_skip(gB, s->mjpb_skiptosod);

    ret = init_get_bits8(gb, s->gB.buffer, bytestream2_get_bytes_left(gB));
    if (ret < 0)
        return ret;
...
static int mjpeg_decode_app(MJpegDecodeContext *s)
{
    ...
    int ret = mjpeg_parse_len(s, &s->gB, &len, "app");
    if (ret < 0)
        return AVERROR_INVALIDDATA;
...
static int mjpeg_decode_com(MJpegDecodeContext *s)
{
    ...
    ret = mjpeg_parse_len(s, gB, &len, "com");
    if (ret < 0)
        return ret;
    if (!len)
        return 0;
    ...
    for (i = 0; i < len; i++)
        cbuf[i] = bytestream2_get_byteu(gB);
    if (cbuf[i - 1] == '\n')
        cbuf[i - 1] = 0;
    else
        cbuf[i] = 0;

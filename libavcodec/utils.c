                                    ...
                                    uint8_t * extradata, int frame_size, int frame_bytes)
{
    ...
    if (frame_bytes > 0) {
        ...
        case AV_CODEC_ID_LHCELP     : d = 160LL * (frame_bytes / 12); break;
        }
        if (d > INT64_MIN)
        ...
        if (ch > 0 && ch < INT_MAX/16) {
            ...
            case AV_CODEC_ID_ADPCM_MTA2:
                return frame_bytes / (16 + ch * 0x90) * 256;
            ...
            if (ba > 0) {
                ...
                case AV_CODEC_ID_ADPCM_XMD:
                    tmp = blocks * 32;
                    break;
                case AV_CODEC_ID_ADPCM_PT:
                    tmp = blocks * (2 + FFMAX(ba/ch - 5, 0) * 2);
                    break;
                case AV_CODEC_ID_ADPCM_FMOD:
                    tmp = blocks * (ba/ch - 0xc) * 2;
                    break;
                case AV_CODEC_ID_ADPCM_BRR:
                    tmp = blocks * 16;
                    break;
                case AV_CODEC_ID_ADPCM_MPC3:
                    tmp = blocks * (ba - 4) / (4 * ch) * 10;
                    break;
                }
                if (tmp) {
                    if (tmp != (int)tmp)

FATE_SAMPLES_AUDIO-$(call TRANSCODE, APTX, APTX, WAV_DEMUXER PCM_S16LE_DECODER ASF2SF_FILTER) += fate-aptx
fate-aptx: CMD = transcode wav $(TARGET_SAMPLES)/audio-reference/luckynight_2ch_44kHz_s16.wav aptx "-af asf2sf -c aptx" "-af asf2sf -c:a pcm_s16le -t 0.25" "" "" "-f aptx -sample_rate 44100"

FATE_SAMPLES_AUDIO-$(call TRANSCODE, APTX_HD, APTX_HD, WAV_DEMUXER PCM_S16LE_DECODER \
                          ASF2SF_FILTER PCM_S32LE_ENCODER) += fate-aptx-hd
fate-aptx-hd: CMD = transcode wav $(TARGET_SAMPLES)/audio-reference/luckynight_2ch_44kHz_s16.wav aptx_hd "-af asf2sf -c aptx_hd" "-af asf2sf -c:a pcm_s32le -t 0.25" "" "" "-f aptx_hd -sample_rate 44100"

FATE_BINKAUDIO-$(call PCM, BINK, BINKAUDIO_DCT, ASF2SF_FILTER) += fate-binkaudio-dct
fate-binkaudio-dct: CMD = pcm -i $(TARGET_SAMPLES)/bink/binkaudio_dct.bik
fate-binkaudio-dct: REF = $(SAMPLES)/bink/binkaudio_dct.pcm
fate-binkaudio-dct: FUZZ = 2

FATE_BINKAUDIO-$(call PCM, BINK, BINKAUDIO_RDFT, ASF2SF_FILTER) += fate-binkaudio-rdft
...
FATE_SAMPLES_AUDIO-$(call PCM, S337M, DOLBY_E, ASF2SF_FILTER) += fate-dolby-e
fate-dolby-e: CMD = pcm -i $(TARGET_SAMPLES)/dolby_e/16-11
fate-dolby-e: CMP = oneoff
fate-dolby-e: REF = $(SAMPLES)/dolby_e/16-11.pcm

FATE_SAMPLES_AUDIO-$(call FRAMECRC, DSS, DSS_SP, ASF2SF_FILTER) += fate-dss-lp
fate-dss-lp: CMD = framecrc -i $(TARGET_SAMPLES)/dss/lp.dss -frames 30 -af asf2sf

FATE_SAMPLES_AUDIO-$(call FRAMECRC, DSS, DSS_SP) += fate-dss-sp
fate-dss-sp: CMD = framecrc -i $(TARGET_SAMPLES)/dss/sp.dss -frames 30

FATE_SAMPLES_AUDIO-$(call PCM, DSF, DST, ASF2SF_FILTER) += fate-dsf-dst
...
FATE_SAMPLES_AUDIO-$(call PCM, FLV, NELLYMOSER, ASF2SF_FILTER) += fate-nellymoser
fate-nellymoser: CMD = pcm -i $(TARGET_SAMPLES)/nellymoser/nellymoser.flv
fate-nellymoser: CMP = oneoff
fate-nellymoser: REF = $(SAMPLES)/nellymoser/nellymoser.pcm

FATE_SAMPLES_AUDIO-$(call ENCMUX, NELLYMOSER, FLV, ASF2SF_FILTER PIPE_PROTOCOL) += fate-nellymoser-aref-encode
...
FATE_SAMPLES_AUDIO-$(call FRAMECRC, AVI, ON2AVC, ASF2SF_FILTER) += fate-on2avc
fate-on2avc: CMD = framecrc -i $(TARGET_SAMPLES)/vp7/potter-40.vp7 -frames 30 -vn -af asf2sf
...
FATE_SAMPLES_AUDIO-$(call FRAMECRC, VMD, VMDAUDIO, ASF2SF_FILTER) += fate-sierra-vmd-audio
fate-sierra-vmd-audio: CMD = framecrc -i $(TARGET_SAMPLES)/vmd/12.vmd -vn -af asf2sf

FATE_SAMPLES_AUDIO-$(call FRAMECRC, SMACKER, SMACKAUD, ASF2SF_FILTER) += fate-smacker-audio
fate-smacker-audio: CMD = framecrc -i $(TARGET_SAMPLES)/smacker/wetlogo.smk -vn -af asf2sf

FATE_SAMPLES_AUDIO-$(call DEMDEC, WSVQA, WS_SND1, ASF2SF_FILTER) += fate-ws_snd
fate-ws_snd: CMD = md5 -i $(TARGET_SAMPLES)/vqa/ws_snd.vqa -f s16le -af asf2sf
